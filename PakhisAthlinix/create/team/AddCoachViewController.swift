...
class AddCoachViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    @objc private func doneButtonTapped() {
        ...
        // Insert selected coaches into Supabase
        Task {
            do {
                ...
                let tempCoachData = tempCoach(
                    teamCoachID: UUID(),
                    createrID: sessionUserID, // Using sessionUserID
                    coach1ID: selectedCoaches.first?.userID ?? UUID(),
                    coach2ID: selectedCoaches.count > 1 ? selectedCoaches[1].userID : UUID()
                )

                try await supabase
                    .from("tempCoach")
                    .insert(tempCoachData)
                    .execute()

                delegate?.didSelectCoaches(selectedCoaches)
                self.dismiss(animated: true)
            } catch {
                print("Error inserting coaches: \(error)")
            }
        }
