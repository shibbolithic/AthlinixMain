...
class AddMembersTableViewController: UITableViewController {
        ...
        private func fetchAthletesData() {
            Task {
                do {
                    ...
                    athletes = try decoder.decode([Usertable].self, from: response.data)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "inviteathletes", for: indexPath)
              let athlete = athletes[indexPath.row]
              
              // Configure cell
              cell.textLabel?.text = athlete.name
              cell.imageView?.image = UIImage(named: athlete.profilePicture!)
