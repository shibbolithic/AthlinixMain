...
class ViewUserViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
            ...
            Task {
                do {
                    await setupPrimaryDataSupabase(forUserID: userID)
                    await fetchTeamsForUserSupabase(userID: userID)
                    await setupProfileDetailsSupabase()
                    await fetchPosts(userID: userID)
                    
                    if let bestMatch = try await fetchBestMatchSupabase(forPlayerID: userID) {
...
extension ViewUserViewController: UICollectionViewDelegate, UICollectionViewDataSource {
